...
document.addEventListener("DOMContentLoaded", function () {
  if (document.querySelectorAll(".indexpage").length > 0) {
    ...
    var scrollDown = function scrollDown() {
      ...
      (function scroll() {
        if (window.pageYOffset < windowCoords && isAnyPartOfElementInViewport(document.querySelector(".header"))) {
    ...
    if (window.innerWidth > tabletwiev) {
      // document.querySelector("#Правила .container").classList.remove("wrapper");
      // document.querySelector("#Правила").classList.add("wrapper");
      document.querySelector(".block-content").classList.add("wrapper");
    ...
    var delay = 100,
        duration = 600; // ScrollReveal({
    //   reset: true
    // }).reveal(
    //   ".login-form,.block-faq,.header .logo,.header .icons,.header .arrow,.navigation-panel .button,.start-game,.block-with-rools",
...
function openSelection(evt) {
  ...
  if (type == "Главная") {
    $(".cards-slider").slick("slickPlay");
    $(".cards-slider").slick("reinit");
    ScrollReveal().sync(); // if (!event.target.classList.contains("slick-slider")) {
    //   $(".cards-slider").slick("resize");
    //   ScrollReveal().sync();
    // } else {
    //   return;
